...
def main() -> None:
    ...
    # Automatic IBM-style GHZ schedule:
    # - choose a connected heavy-hex-like patch
    # - generate source/target/layer lists
    # - insert one 300 ns delay after each entangling layer
    num_roots_to_try = 80
    ...
    print(f"Running comparison sweeps with shots={shots}, N={N}")
    print(
    ...
    # Measurement error sweep (adaptive protocols only)
    full_meas = []
    semi_meas = []
    for i, p in enumerate(p_vals):
        ...
        full_meas.append(F_full)
        semi_meas.append(F_semi)
        print(f"p_meas={p:.4f} -> full={F_full:.6f}, semi={F_semi:.6f}")

    plt.figure(figsize=(7.5, 3.5))
    plt.plot(p_vals, full_meas, marker="o", label="Full adaptive")
    plt.plot(p_vals, semi_meas, marker="s", label="Semi-adaptive")
    plt.xlabel("Measurement error probability")
    plt.ylabel("GHZ fidelity")
    plt.title(f"Fidelity vs measurement error (shots={shots}, N={N})")
    plt.grid(True, alpha=0.35)
    plt.legend(loc="lower left")
    plt.tight_layout()
    fig_meas = outdir / f"protocol_comparison_fidelity_vs_measurement_error_{N}N.png"
    plt.savefig(fig_meas, dpi=180)
    plt.close()

    # CNOT sweep (all three)
    full_cx = []
    semi_cx = []
    non_cx = []
    for i, p in enumerate(p_vals):
        ...
        full_cx.append(F_full)
        semi_cx.append(F_semi)
        non_cx.append(F_non)
        print(f"p_cx={p:.4f} -> full={F_full:.6f}, semi={F_semi:.6f}, non={F_non:.6f}")

    plt.figure(figsize=(7.5, 3.5))
    plt.plot(p_vals, full_cx, marker="o", label="Full adaptive")
    plt.plot(p_vals, semi_cx, marker="s", label="Semi-adaptive")
    plt.plot(p_vals, non_cx, marker="^", label="Non-adaptive")
    plt.xlabel("CX gate error probability")
    plt.ylabel("GHZ fidelity")
    plt.title(f"Fidelity vs CX gate error (shots={shots}, N={N})")
    plt.grid(True, alpha=0.35)
    plt.legend(loc="lower left")
    plt.tight_layout()
    fig_cx = outdir / f"protocol_comparison_fidelity_vs_entangling_gate_error_{N}N.png"
    plt.savefig(fig_cx, dpi=180)
    plt.close()
    ...
    for i, T in enumerate(T_vals):
        ...
        print(f"Tphi={T:.3f} -> full={F_full:.6f}, semi={F_semi:.6f}, non={F_non:.6f}")
    ...
    plt.tight_layout()
    fig_tphi = outdir / f"protocol_comparison_fidelity_vs_tphi_{N}N.png"
    ...
    for i, T in enumerate(T_vals):
        ...
        F_non = fidelity_non_adaptive(
            ...
            T1=T, Tphi=float("inf"),
            p_cx=0.0,
            seed=seed + 10000 + 10 * i,
        )
        full_t1.append(F_full)
        semi_t1.append(F_semi)
        non_t1.append(F_non)
        print(f"T1={T:.3f} -> full={F_full:.6f}, semi={F_semi:.6f}, non={F_non:.6f}")

    plt.figure(figsize=(7.5, 3.2))
    plt.plot(T_vals, full_t1, marker="o", label="Full adaptive")
    plt.plot(T_vals, semi_t1, marker="s", label="Semi-adaptive")
    plt.plot(T_vals, non_t1, marker="^", label="Non-adaptive")
    plt.xscale("log")
    plt.xlabel(r"$T_1$ (ns)")
    plt.ylabel("GHZ fidelity")
    plt.title(fr"Fidelity vs $T_1$ (shots={shots}, N={N})")
    plt.grid(True, alpha=0.35)
    plt.legend(loc="upper left")
    plt.tight_layout()
    fig_t1 = outdir / f"protocol_comparison_fidelity_vs_t1_{N}N.png"
    plt.savefig(fig_t1, dpi=180)
    plt.close()
